...
[ScopeAware]
public sealed record UserNotificationRequestedIntegrationEventV2 : IntegrationEvent, IScopedIntegrationEvent
{
    public const string EventType = UserNotificationRequestedIntegrationEvent.EventType;
    public const int EventVersion = 2;

    public UserNotificationRequestedIntegrationEventV2(
        Guid eventId,
        string scopeId,
        ...
        : base(eventId, occurredAtUtc, EventType, EventVersion)
    {
        ...
        this.Title = NormalizeText(title, UserNotificationRequestedIntegrationEvent.TitleMaxLength, nameof(title));
        this.Body = string.IsNullOrWhiteSpace(body)
            ? null
            : NormalizeText(body, UserNotificationRequestedIntegrationEvent.BodyMaxLength, nameof(body));
    ...
    private static System.Collections.ObjectModel.ReadOnlyCollection<NotificationTag> CopyTags(
        IReadOnlyList<NotificationTag> tags)
    {
        ArgumentNullException.ThrowIfNull(tags);
        NotificationTag[] normalized = tags
            .Select(tag => tag ?? throw new ArgumentException("Notification tags cannot contain null values.", nameof(tags)))
        ...
        {
            normalized = [new NotificationTag(NotificationTags.Web, NotificationTagKind.Delivery)];
        }
        ...
        {
            throw new ArgumentException("Notifications must contain at least one delivery tag.", nameof(tags));
        }
    ...
    private static string NormalizeSourceModule(string sourceModule)
    {
        ...
        return normalized.Length <= UserNotificationRequestedIntegrationEvent.SourceModuleMaxLength
            ? normalized
            : throw new ArgumentException(
    ...
    private static string NormalizeNotificationName(string notificationName)
    {
        ...
        return normalized.Length <= UserNotificationRequestedIntegrationEvent.NameMaxLength
            ? normalized
            : throw new ArgumentException(
    ...
    private static NotificationSeverity NormalizeSeverity(NotificationSeverity severity) =>
        severity is not NotificationSeverity.Unknown && Enum.IsDefined(severity)
            ? severity
            : throw new ArgumentOutOfRangeException(nameof(severity), severity, "Notification severity must be defined and non-unknown.");

    private static NotificationDeliveryPolicy NormalizePolicy(NotificationDeliveryPolicy policy) =>
        policy is NotificationDeliveryPolicy.RespectPreferences or NotificationDeliveryPolicy.Mandatory
            ? policy
            : throw new ArgumentOutOfRangeException(nameof(policy), policy, "Notification delivery policy is invalid.");

    private static string NormalizePayloadJson(string payloadJson)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(payloadJson);
        if (payloadJson.Length > UserNotificationRequestedIntegrationEvent.PayloadJsonMaxLength)
        {
            throw new ArgumentException(
        ...
        try
        {
            ...
            return normalizedJson.Length <= UserNotificationRequestedIntegrationEvent.PayloadJsonMaxLength
                ? normalizedJson
                : throw new ArgumentException(
                    $"payloadJson must be {UserNotificationRequestedIntegrationEvent.PayloadJsonMaxLength} characters or fewer.",
                    nameof(payloadJson));
