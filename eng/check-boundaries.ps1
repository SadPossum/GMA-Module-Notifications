...
foreach ($projectFile in $projectFiles) {
    ...
    foreach ($reference in $project.SelectNodes('//ProjectReference')) {
        ...
        if ($include -match '\$\(GmaModule(?!NotificationsRoot\))') {
...
foreach ($sourceFile in $sourceFiles) {
    ...
    if ($source -match 'Gma\.Modules\.(?!Notifications(?:\.|;))') {
    ...
    if ($relativePath -match '^src\\Gma\.Modules\.Notifications\.(?:Api|AdminApi)\\' -and
        $source -match 'Gma\.Modules\.Notifications\.Domain') {
        $errors.Add("$relativePath crosses the front-door to domain boundary.")
    }
}

if ($errors.Count -gt 0) {
    throw "Notifications boundary checks failed:`n - $($errors -join "`n - ")"
}

Write-Host 'Notifications boundary checks passed.'
